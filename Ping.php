...
<h1>
    ...
    <script>
        ...
        function processFrame(hands) {
            ...
            // Обрабатываем взаимодействие с одной рукой
            if (hands.length === 1) {
                const hand = hands[0];
                const indexFinger = hand.indexFinger;
                ...
                const lineVector = vecSubtract(thumb, indexFinger);
                const normalizedLine = vecNormalize(lineVector);

                const ballToLineStart = vecSubtract(ballPos, indexFinger);
                const projectionLength = vecDot(ballToLineStart, normalizedLine);
                const projectionPoint = vecAdd(indexFinger, vecMultiply(normalizedLine, projectionLength));
        ...
        hands.setOptions({
            maxNumHands: 1, // Изменяем на одну руку
            modelComplexity: 1,
        ...
        hands.onResults(results => {
            const landmarks = results.multiHandLandmarks.map(handLandmarks => {
                return {
                    indexFinger: {
                        x: (1 - handLandmarks[8].x) * width, // Учитываем зеркальность для указательного пальца
                        y: handLandmarks[8].y * height
                    },
                    thumb: {
                        x: (1 - handLandmarks[4].x) * width, // Учитываем зеркальность для большого пальца
                        y: handLandmarks[4].y * height
                    }
                };
            });
